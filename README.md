...
| FR-12a 按月重复记录 | 新增事件 / 批注均可勾选「按月重复」：选星期几并多选月份（支持跨年切换），实时预览后一次展开逐日写入；重复组可整组修改内容、星期与月份，也可解除为单条后单独修改 |
...
- 按月重复已支持事件与批注展开为逐日条目，并支持整组编辑（内容 / 星期 / 月份，自动增删日期）；多设备同步、作者维度统计仍为后续扩展项。旧版本已展开的数据（非导入、同内容同星期）加载时会自动补组标记；删除仍按单条进行。
